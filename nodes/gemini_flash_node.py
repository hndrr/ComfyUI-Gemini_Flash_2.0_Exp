...
class GeminiFlash:
    ...
    def prepare_content(
        ...
        if input_type == "text":
            text_content = (
                prompt if not Additional_Context else f"{prompt}\n{Additional_Context}"
            )
            ...
            for i in range(batch_count):
                try:
                    ...
                    if batch_images:
                        ...
                        status_text += (
                            f"Batch {i+1}: Generated {len(batch_images)} images\n"
                        )
                    else:
                ...
                if input_type == "text":
                    ...
                    if len(video.shape) == 4 and video.shape[0] > 1:
                        ...
                        if frames:
                            parts = [
                                {
                                    "text": f"This is a video with {frame_count} frames. {prompt}"
                                }
                            ]
                    ...
                    if isinstance(content_parts, list) and len(content_parts) > 0:
                        if "parts" in content_parts[0]:
                            for part in content_parts[0]["parts"]:
                                if "text" in part:
                                    part["text"] = (
                                        f"Please provide the response in a structured format. {part['text']}"
                                    )
